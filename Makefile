...
.PHONY: clean install test start

all:
	NODE_ENV=foo npm install
	export MOCHA_REPORTER_STACK_EXCLUDE=node_modules
	@NODE_ENV=test npm test && cat ./coverage/lcov.info | CODACY_PROJECT_TOKEN=JGfC5w1O8e9b09V7I76P ./node_modules/.bin/codacy-coverage && rm -rf ./coverage
...
cover:
	clear
	cat ./coverage/lcov.info | CODACY_PROJECT_TOKEN=JGfC5w1O8e9b09V7I76P ./node_modules/.bin/codacy-coverage
